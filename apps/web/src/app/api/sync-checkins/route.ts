...
import type { PendingSyncRecord } from '@/types';

export async function POST(request: NextRequest) {
  if (!request.body) {
    console.warn('Sync endpoint sin cuerpo');
    return NextResponse.json({ message: 'Payload requerido' }, { status: 400 });
  }

  let payload: PendingSyncRecord | undefined;
  try {
    payload = (await request.json()) as PendingSyncRecord;
  } catch (error) {
    console.error('Sync payload inválido', error);
  ...
  try {
    endpoint = new URL(getFunctionsUrl('/sync-checkins'));
  } catch (error) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Error remoto sync', response.status, message);
      return NextResponse.json({ message: 'Error remoto al sincronizar' }, { status: response.status });
    }

    const data = await response.json();
    return NextResponse.json(data);
  } catch (error) {
    console.error('Fallo llamando función de sync', error);
